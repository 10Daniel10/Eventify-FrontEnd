...
export const CustomLink: FC<CustomLinkI> = ({ children, className, customVariant, customColor, href, onclick, underline = 'none' }: CustomLinkI) => {
  ...
  return (
    <Link onClick={onclick} className={linkClass} href={href} underline={underline}>{children}</Link>
  )
}